// Regenerates src/styles/palette.css.
//
// The accent (blue) and gray scales are the company-approved default palette
// (see palette/light and palette/dark) and are pinned here verbatim rather
// than pulled from @radix-ui/colors, because the installed package's "blue"
// and "gray" scales have since drifted from the approved swatches. The
// semantic status scales (red/green/amber) aren't brand-mandated, so those
// are still generated from @radix-ui/colors.
//
// Run with: npm run generate:palette

import { writeFile } from "node:fs/promises";
import { fileURLToPath } from "node:url";
import * as colors from "@radix-ui/colors";

const OUTPUT = fileURLToPath(new URL("../src/styles/palette.css", import.meta.url));

// --- Approved brand palette (pinned) -----------------------------------
...
// --- Generated semantic status scales (not brand-mandated) -------------

const GENERATED_SCALE_PREFIXES = ["red", "green", "amber"];

function toDeclarations(scale, prefix, { alpha = false } = {}) {
  return Object.entries(scale)
    .map(([key, value]) => {
      const step = key.match(/\d+$/)?.[0];
      const varName = alpha ? `--${prefix}-a${step}` : `--${prefix}-${step}`;
      return `  ${varName}: ${value};`;
    })
    .join("\n");
}

function generatedSolid({ dark = false } = {}) {
  return GENERATED_SCALE_PREFIXES.flatMap((prefix) => {
    const solidExport = dark ? `${prefix}Dark` : prefix;
    const alphaExport = dark ? `${prefix}DarkA` : `${prefix}A`;
    return [
      toDeclarations(colors[solidExport], prefix),
      toDeclarations(colors[alphaExport], prefix, { alpha: true }),
    ];
  }).join("\n");
}

function generatedP3({ dark = false } = {}) {
  return GENERATED_SCALE_PREFIXES.flatMap((prefix) => {
    const solidExport = dark ? `${prefix}DarkP3` : `${prefix}P3`;
    const alphaExport = dark ? `${prefix}DarkP3A` : `${prefix}P3A`;
    return [
      toDeclarations(colors[solidExport], prefix).replace(/^ {2}/gm, "      "),
      toDeclarations(colors[alphaExport], prefix, { alpha: true }).replace(/^ {2}/gm, "      "),
    ];
  }).join("\n");
}

function themeBlock(selector, theme) {
  const solid = `${selector} {\n${BRAND[theme].solid}\n${generatedSolid({ dark: theme === "dark" })}\n}\n`;
  const p3 = `@supports (color: color(display-p3 1 1 1)) {
  @media (color-gamut: p3) {
    ${selector} {
${BRAND[theme].p3}
${generatedP3({ dark: theme === "dark" })}
    }
  }
}
`;
 ...
 * Do not edit by hand — run \`npm run generate:palette\` instead.
 *
 * The accent (blue) and gray scales are pinned to the company-approved
 * default palette; red/green/amber are generated from @radix-ui/colors.
 */
...
out += "\n";
out += themeBlock('[data-theme="dark"]', "dark");
